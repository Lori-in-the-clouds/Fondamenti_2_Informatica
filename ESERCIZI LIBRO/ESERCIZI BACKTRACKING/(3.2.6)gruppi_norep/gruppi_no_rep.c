...
void GruppiNoRepRec(const struct alunno* p, int n, int* nsol, struct alunno* vcurr, int i, int size, int vcurr_size) {
	if (vcurr_size == n) {
		(*nsol)++;
		printf("{");
		for (int j = 0; j < n - 1; j++) {
	...
	if (Verifica(p + i, vcurr, vcurr_size)) {
		strcpy(vcurr[vcurr_size].name, p[i].name);
		GruppiNoRepRec(p, n, nsol, vcurr, i + 1, size, vcurr_size + 1);
	}
		GruppiNoRepRec(p, n, nsol, vcurr, i + 1, size, vcurr_size);
}
int GruppiNoRep(const char* filename, int n) {
	...
	GruppiNoRepRec(p, n, &nsol, vcurr, 0, size - 1,0);
	return nsol;
}